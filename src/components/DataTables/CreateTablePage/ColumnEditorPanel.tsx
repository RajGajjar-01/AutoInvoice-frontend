...
interface ColumnCardProps {
  ...
  isOpen: boolean
  onToggle: () => void
  onChange: (index: number, updated: Column) => void
    ...
    >
      ...
      <div className="flex items-center group">
        ...
        >
          ...
          {/* Column name */}
          <span
            className={`flex-1 min-w-0 text-sm font-medium truncate ${col.name.trim() ? "text-foreground" : "text-muted-foreground/50 italic"}`}
          >
            {displayName}
          </span>
          ...
          {/* Type badge */}
          <span
            className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold whitespace-nowrap ${badgeColor}`}
          >
            {col.type}
          </span>
      ...
      {isOpen && (
        <div className="px-3 pb-3 flex flex-col gap-3 border-t border-border">
          ...
          <div className="flex flex-col gap-1 pt-2">
            ...
            {col._isDuplicate && (
              <p className="text-[11px] text-destructive">Duplicate name</p>
            )}
          </div>
...
export function ColumnEditorPanel({
  ...
  const handleChange = (index: number, updated: Column) => {
      ...
      <div className="px-4 py-3 border-b border-border shrink-0 flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Click a column to configure it
          </p>
        </div>
            ...
            <div className="flex flex-col gap-1.5 px-3 py-3">
              {enrichedColumns.map((col, i) => (
                <ColumnCard
                  ...
                  onToggle={() =>
                    setOpenId((prev) => (prev === col._id ? null : col._id))
                  }
