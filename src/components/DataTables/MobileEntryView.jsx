...
import { Plus, Trash2, Edit2, CheckCircle2, ChevronDown, ChevronUp, Bell } from "lucide-react"
            ...
            <Card className="border border-primary/20 shadow-sm overflow-hidden bg-background">
                ...
                >
                    ...
                    <div className="p-1 rounded-full bg-background/50 border border-border/20">
                        {adding ? <ChevronUp className="h-3 w-3 text-muted-foreground" /> : <ChevronDown className="h-3 w-3 text-muted-foreground" />}
                    </div>
                </CardHeader>
                {adding && (
                    <CardContent className="p-0 flex flex-col divide-y divide-border/30 border-t border-border/30">
                        {cols.map((col) => (
                            <div key={col.name} className="grid grid-cols-[85px_1fr] items-center px-3 py-1 gap-2 min-h-[36px]">
                                ...
                                <div className="min-h-[30px] flex items-center bg-muted/5 rounded border border-border/20 overflow-hidden">
                                    <TableCell
                                        ...
                                        onChange={(val) => setNewItem(prev => ({ ...prev, [col.name]: val }))}
                ...
                {adding && (
                    <CardFooter className="p-2 bg-muted/10 border-t border-border/40 flex gap-2">
                        <Button className="flex-1 h-8 text-xs font-bold" onClick={handleAdd}>
                            Add Entry
                        </Button>
                        <Button variant="ghost" className="h-8 px-3 text-xs" onClick={() => setAdding(false)}>
                            Cancel
                        </Button>
                    </CardFooter>
                )}
                    ...
                    [...rows].reverse().map((row, revIdx) => {
                        ...
                        return (
                            <MobileRowCard
                                ...
                                onCellChange={(colName, val) => onUpdateCell(row.id, colName, val)}
...
function MobileRowCard({ row, cols, idx, onDelete, onCellChange, suggestionsMap, onSetReminder }) {
        ...
        <Card className="shadow-sm border border-border/60 overflow-hidden bg-card transition-all active:scale-[0.99] hover:border-primary/30">
            ...
            >
                <div className="flex flex-col overflow-hidden">
                    <span className="text-xs font-bold truncate text-foreground/90">{primaryTitle}</span>
                    <span className="text-[10px] text-muted-foreground font-medium italic truncate">#{idx + 1} Entry</span>
                </div>
            ...
            {expanded && (
                <CardContent className="px-3 pb-3 pt-1 flex flex-col gap-0 divide-y divide-border/20 bg-muted/10 border-t border-border/40">
                    {cols.map((col) => (
                        <div key={col.name} className="grid grid-cols-[100px_1fr] items-center py-2 px-1 gap-2">
