...
const makeColumn = (name = "", type = "Text"): Column => ({
  name,
  type,
  mandatory: false,
  options: [],
...
interface CreateTableModalProps {
  ...
  onSubmit: (data: { name: string; columns: Column[] }) => void
  mode?: "blank" | "template"
  initialName?: string
...
export function CreateTableModal({
  ...
  const getInitialCols = () =>
    initialColumns.length > 0
      ? initialColumns.map((c) => makeColumn(c.name, c.type))
      : [makeColumn()]

  const [columns, setColumns] = useState<Column[]>(getInitialCols)
  ...
  }, [open, initialColumns, initialName, form.reset])
  ...
  // ── Quick-add ─────────────────────────────────────────────────────────────
  const handleQuickAdd = (preset: { name: string; type: string }) => {
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Invoice Tracker" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
