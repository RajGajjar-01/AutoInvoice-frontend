...
export function ItemDetail({ item, onDeleted }) {
    const stock = item.stock ?? 0
    const threshold = item.lowStockThreshold ?? 5
    const margin = item.purchasePrice && item.salePrice
        ? (((item.salePrice - item.purchasePrice) / item.salePrice) * 100).toFixed(1)
        : null

    const stockValueClass = stock === 0
        ? "text-destructive"
        : stock < threshold
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <StatCard
                    ...
                    sub={stock < threshold && stock > 0 ? "Below threshold" : stock === 0 ? "Reorder needed" : ""}
                ...
                {margin != null && (
                    <StatCard
                        ...
                        value={`${margin}%`}
                        sub="on sale price"
                        valueClass={Number(margin) >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-destructive"}
                    />
                )}
            </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <Card>
                        ...
                        <CardContent className="pt-2 divide-y divide-border">
                            ...
                            <ProfileRow icon={Barcode} label="SKU / Item Code" value={item.sku} mono />
                            <ProfileRow icon={Layers} label="Unit of Measure" value={item.unit} />
                            <ProfileRow icon={BadgeIndianRupee} label="Sale Price" value={item.salePrice != null ? `₹${Number(item.salePrice).toLocaleString("en-IN")}` : null} />
                            <ProfileRow icon={ShoppingCart} label="Cost Price" value={item.purchasePrice != null ? `₹${Number(item.purchasePrice).toLocaleString("en-IN")}` : null} />
                            <ProfileRow icon={Percent} label="GST Rate" value={item.taxRate != null ? `${item.taxRate}%` : null} />
                            <ProfileRow icon={FileCode} label="HSN / SAC Code" value={item.hsnCode} mono />
                            <ProfileRow icon={Info} label="Description" value={item.description} />
                        </CardContent>
                    </Card>
                </div>
            </div>
