...
export function CreateInvoicePage({ defaultType = "invoice" }) {
  ...
  const { customerId: preselectedCustomerId, itemId: preselectedItemId, type: preselectedType, fromId, fromType } = useSearch({ strict: false })
  ...
  const handleItemSelect = (index, itemId) => {
    ...
    if ((invItem.stock || 0) <= 0) {
      showErrorToast(`Warning: ${invItem.name} is out of stock (Stock: 0). You can still add it, but remember to update your inventory.`)
    }
  ...
  const handleSave = () => {
    ...
    // Checking for out-of-stock items before final save
    const outOfStockItems = items.filter(it => {
      if (!it.itemId) return false
      const inv = inventoryItems.find(i => i.id === it.itemId)
      return inv && (inv.stock || 0) <= 0
    })

    if (outOfStockItems.length > 0) {
      // We don't block strictly as per user request ("could still be able to add"), but we alert
      const names = outOfStockItems.map(i => i.name).join(", ")
      const confirmSave = window.confirm(`Warning: The following items are out of stock: ${names}. Do you still want to save?`)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-1">
        <Label className="text-sm font-medium text-muted-foreground whitespace-nowrap">Document Type:</Label>
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <Button
              key={type.id}
              variant={docType === type.id ? "default" : "secondary"}
              size="sm"
              onClick={() => handleTypeChange(type.id)}
              className={`rounded-full px-4 h-8 text-xs font-semibold transition-all ${docType === type.id ? "shadow-sm ring-1 ring-primary/20" : "bg-muted/50 hover:bg-muted"}`}
            >
              {type.label}
            </Button>
          ))}
