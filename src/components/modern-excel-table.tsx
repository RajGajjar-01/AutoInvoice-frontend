...
export function ModernExcelTable({
  ...
  updateItem: (index: number, field: string, value: string | number | boolean) => void
                    ...
                    <div className="flex flex-col gap-1.5 w-full">
                      <div className="flex items-center border border-border/10 rounded-md focus-within:border-primary/20 bg-muted/5 transition-all w-full overflow-hidden group/itembox">
                        ...
                        >
                          ...
                          <SelectContent align="end" className="w-[220px]" position="popper">
                      ...
                      {item.hsnCode && (
                        <div className="flex items-center gap-2 px-2.5 pb-1">
                          <button
                            type="button"
                            onClick={() => updateItem(index, "showHsn", item.showHsn ? 0 : 1)}
                          ...
                          >
                            <span
                              className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow transition-transform duration-200 ${
                                item.showHsn ? "translate-x-4" : "translate-x-0.5"
                              }`}
                            />
                          </button>
                          <span className="text-[10px] text-muted-foreground">
                            ...
                            <span className={`ml-1 font-mono font-semibold ${
                              item.showHsn ? "text-foreground" : "line-through opacity-40"
                            }`}>
                              {item.hsnCode}
                            </span>
                            <span className="ml-1 opacity-50">
                              {item.showHsn ? "(included in invoice)" : "(excluded)"}
                            </span>
                          </span>
                        </div>
                      )}
