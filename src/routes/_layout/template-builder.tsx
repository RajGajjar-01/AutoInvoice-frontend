...
const ACCENT_COLOURS: ColorOption[] = [
...
const TEXT_COLOURS: ColorOption[] = [
  { label: "Black", value: "#111827" },
...
interface GlobalStyle {
  ...
  fontFamily?: string
  pageBg?: string
  headerStyle?: "filled" | "minimal" | "underline" | "dark"
  invoiceTitle?: string
}
...
function BlockPreview({ type, globalStyle = {} }: BlockPreviewProps) {
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ color: "#6b7280", fontSize: 11, margin: 0 }}>
                123 Business Park, Mumbai · GSTIN: 27ABC1234Z
              </p>
            </div>
          </div>
        </div>
    ...
    case "customer-details":
      return (
        <div style={base}>
          ...
          <p style={{ fontWeight: 700, margin: "0 0 1px" }}>
            ABC Technologies Ltd
          </p>
          <p style={{ color: "#6b7280", fontSize: 11, margin: 0 }}>
            456 Tech Street, Bangalore · GSTIN: 29XYZ5678A
          </p>
        </div>
      )
    case "invoice-meta":
      return (
        <div
          style={{ ...base, display: "flex", justifyContent: "space-between" }}
        >
          <div>
            <p style={{ margin: "0 0 2px" }}>
              <span style={{ color: "#6b7280" }}>Invoice # </span>
              <strong>INV-2603-001</strong>
            </p>
            <p style={{ margin: 0 }}>
      ...
      return (
        <div style={base}>
          ...
          <p style={{ color: "#6b7280", margin: 0, fontSize: 11 }}>
            Net 30 days · Late fee: 2% per month
          </p>
        </div>
      )
    case "signature":
...
function SortableCanvasBlock({
  ...
  return (
    <div
      ...
      className={`relative rounded-lg border bg-card transition-all cursor-pointer group ${
        isSelected
    ...
    >
      ...
      <div className="flex items-center gap-1.5 px-2.5 py-1 bg-muted/40 border-b border-border rounded-t-lg">
        ...
        {def && (
          <span className={`rounded p-0.5 ${def.accent}`}>
...
interface BlockStylePanelProps {
  block: Block | null
  onChange: (key: keyof BlockStyle, value: string | boolean) => void
}

function BlockStylePanel({ block, onChange }: BlockStylePanelProps) {
  if (!block)
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center px-2">
        ...
        <p className="text-xs text-muted-foreground">
          Click a canvas block to edit its style
        </p>
      </div>
    )
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 pb-2 border-b border-border">
        <span className={`rounded p-1 ${def?.accent || "bg-muted"}`}>
        ...
        <div className="grid grid-cols-3 gap-1">
          {[
            ...
            <button
              ...
              style={{ background: opt.value || "var(--card)" }}
            >
              {(s.bgColor || "") === opt.value && (
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <span
            className={`absolute top-0.5 w-4 h-4 rounded-full bg-background shadow transition-all ${s.border ? "left-4" : "left-0.5"}`}
          />
        </button>
      </div>
...
interface GlobalStylePanelProps {
  globalStyle: GlobalStyle
  onChange: (key: keyof GlobalStyle, value: string) => void
}

function GlobalStylePanel({ globalStyle, onChange }: GlobalStylePanelProps) {
  return (
    <div className="space-y-4">
      {/* Font family */}
      <div className="space-y-1">
        ...
        <Select
          value={globalStyle.fontFamily || FONT_FAMILIES[0].value}
      ...
      <div className="space-y-1">
        ...
        <div className="grid grid-cols-4 gap-1">
          {ACCENT_COLOURS.map((c) => (
            <button
              ...
              className={`h-7 rounded border-2 transition-all ${globalStyle.accentColor === c.value ? "ring-2 ring-offset-1 ring-primary border-background scale-110" : "border-transparent"}`}
        ...
        <div className="flex items-center gap-1.5 mt-1">
          <div
            ...
            style={{ background: globalStyle.accentColor || "#16a34a" }}
          />
          <Input
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-1.5">
          <div
            className="w-6 h-6 rounded border"
            style={{ background: globalStyle.textColor || "#111827" }}
          />
          <Input
            className="h-6 text-xs font-mono"
      ...
      <div className="space-y-1">
        ...
        <Select
          value={globalStyle.headerStyle || "filled"}
          onValueChange={(v) => onChange("headerStyle", v)}
        >
          <SelectTrigger className="h-7 text-xs">
            <SelectValue />
          </SelectTrigger>
...
function TemplateBuilderPage() {
  ...
  const [leftTab, setLeftTab] = useState<"blocks" | "global" | "block-style">(
    "blocks",
  )
  ...
  const handleDragEnd = useCallback(
    (event: {
      ...
      if (event.over && event.active.id !== event.over.id) {
        setBlocks((prev) => {
          const from = prev.findIndex((b) => b.id === String(event.active.id))
          const to = prev.findIndex((b) => b.id === String(event.over?.id))
  ...
  const handleAddBlock = useCallback((type: string) => {
    const newBlock: Block = { id: `blk-${Date.now()}`, type, style: {} }
  ...
  const handleStyleChange = useCallback(
    (key: keyof BlockStyle, value: string | boolean) => {
  ...
  const handleGlobalChange = useCallback(
    (key: keyof GlobalStyle, value: string) => {
  ...
  const handleSave = useCallback(() => {
    ...
    p.then(async (createdOrUpdated: { id?: string } | undefined) => {
      const id = createdOrUpdated?.id ?? serverCustomTemplate?.id
      if (id) {
        await activateTemplateMutation.mutateAsync({ id })
      }
      showSuccessToast(
        "Template saved and activated! It will be used for all new invoices.",
      )
    }).catch(() => {
  ...
  const handlePreview = useCallback(() => {
    ...
    const blockHtml = (block: Block): string => {
      const s = block.style || {}
      const fs =
        s.fontSize === "xs" ? "11px" : s.fontSize === "base" ? "15px" : "13px"
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:${font};color:${textColor};background:${pageBg};padding:32px}@media print{body{padding:12mm}}</style>
    ...
    const popup = window.open(
      "",
      "_blank",
      "width=960,height=720,left=100,top=80",
    )
    if (!popup) {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-xs">
              Drag to reorder · click block to style it · save to use as your
              invoice template
            </p>
          </div>
        </div>
      ...
      >
        ...
        <Card className="flex flex-col overflow-hidden">
          ...
          <div className="flex border-b border-border shrink-0">
            {[
              { id: "blocks" as const, icon: Layout, label: "Fields" },
              { id: "global" as const, icon: Palette, label: "Design" },
                ...
                {BLOCK_DEFS.map((def) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Plus
                        className={`h-3.5 w-3.5 shrink-0 ${alreadyAdded ? "text-muted-foreground/40" : "text-muted-foreground opacity-0 group-hover:opacity-100"}`}
                      />
                    </button>
                  )
                })}
            ...
            >
              ...
              >
                ...
                <DragOverlay dropAnimation={null}>
                  {dragActiveBlock && (
                    <div
                      className="rounded-lg border border-primary bg-card shadow-2xl opacity-90 pointer-events-none"
                      style={{ maxWidth: 640 }}
                    >
                      <div className="flex items-center gap-2 px-2.5 py-1 bg-muted/40 border-b border-border rounded-t-lg">
                        ...
                        <span className="text-xs font-medium">
                          {
                            BLOCK_DEFS.find(
                              (d) => d.type === dragActiveBlock.type,
                            )?.label
                          }
                        </span>
                      </div>
                      <BlockPreview
                        type={dragActiveBlock.type}
                        globalStyle={globalStyle}
                      />
                    </div>
                  )}
