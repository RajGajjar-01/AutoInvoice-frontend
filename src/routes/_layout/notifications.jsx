...
// ─── Type config ──────────────────────────────────────────────────────────────
const TYPE_CONFIG = {
  reminder: { label: "Reminder", Icon: Bell, color: "text-primary", bg: "bg-primary/10" },
  due_date: { label: "Due Date", Icon: Calendar, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-950/40" },
  expiry: { label: "Expiry", Icon: Clock, color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-950/40" },
  info: { label: "Info", Icon: Info, color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-950/40" },
}
function getType(t) { return TYPE_CONFIG[t] ?? TYPE_CONFIG.info }

// ─── Stat card ────────────────────────────────────────────────────────────────
function StatCard({ label, value, icon: Icon, iconClass, bgClass, active }) {
  return (
    <div className={`flex items-center gap-3 rounded-xl border px-4 py-3.5 transition-all ${active ? "border-primary/40 bg-primary/5 dark:bg-primary/10 shadow-sm" : "border-border bg-card"
      }`}>
...
function NotificationCard({ notif, onMarkRead, onDelete }) {
  ...
  return (
    <div
      className={`group relative flex gap-3.5 rounded-xl border px-4 py-3.5 transition-all cursor-default ${isUnread
    ...
    >
      ...
      <div className="flex-1 min-w-0 pr-16">
        ...
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          <p className={`text-sm leading-snug ${isUnread ? "font-semibold text-foreground" : "font-medium text-foreground/75"}`}>
            {notif.title}
          </p>
          {isUnread && (
        ...
        <div className="flex items-center gap-1.5 mt-2 flex-wrap">
          ...
          <span className="text-[10px] text-muted-foreground/60 ml-auto">{formatTimeAgo(notif.createdAt)}</span>
        </div>
      </div>
      ...
      <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        {isUnread && (
          <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary hover:bg-primary/10"
            title="Mark as read" onClick={() => onMarkRead(notif.id)}>
        ...
        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive hover:bg-destructive/10"
          title="Remove" onClick={() => onDelete(notif.id)}>
...
function GroupedList({ items, onMarkRead, onDelete }) {
  ...
  for (const n of items) {
    ...
    if (!seen.has(dateKey)) {
      ...
      groups.push({ dateKey, label: formatDateLabel(n.createdAt + "T00:00:00"), items: [] })
    }
      ...
      {groups.map((g) => (
        <div key={g.dateKey} className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2">
            {g.items.map((n) => (
              <NotificationCard key={n.id} notif={n} onMarkRead={onMarkRead} onDelete={onDelete} />
            ))}
          </div>
        </div>
      ))}
    ...
    <div className="flex flex-col items-center justify-center gap-3 py-16 text-center rounded-xl border border-dashed border-border bg-muted/10">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5 max-w-[220px] mx-auto">
          {filtered ? "Try adjusting your search." : "Notifications from reminders will appear here."}
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        {totalCount > 0 && (
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <Button id="notif-mark-all-btn" variant="outline" size="sm" className="gap-1.5 text-xs" onClick={handleMarkAllRead}>
                <CheckCheck className="h-3.5 w-3.5" />
                Mark all read
              </Button>
            )}
            <Button id="notif-clear-all-btn" variant="outline" size="sm"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <StatCard label="Total" value={totalCount} icon={Bell} iconClass="text-primary" bgClass="bg-primary/10" active={false} />
        <StatCard label="Unread" value={unreadCount} icon={Bell} iconClass="text-amber-500" bgClass="bg-amber-50 dark:bg-amber-950/40" active={unreadCount > 0} />
        <StatCard label="Read" value={readCount} icon={Check} iconClass="text-green-600" bgClass="bg-green-50 dark:bg-green-950/40" active={false} />
      </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {totalCount > 0 && (() => {
            const typeEntries = Object.entries(TYPE_CONFIG)
              .map(([key, cfg]) => ({ key, cfg, count: notifications.filter((n) => n.type === key).length }))
              .filter((e) => e.count > 0)
                ...
                {typeEntries.map(({ key, cfg, count }) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-muted-foreground flex-1 text-xs">{cfg.label}</span>
          ...
          {totalCount === 0 && (
            <div className="rounded-xl border border-border bg-card p-4 flex flex-col gap-3">
              ...
              {[
                { icon: Bell, color: "text-primary", bg: "bg-primary/10", text: "Open any Data Table and click the 🔔 bell icon on a row" },
                { icon: Calendar, color: "text-rose-500", bg: "bg-rose-50 dark:bg-rose-950/40", text: "Set a reminder date and title" },
                { icon: Check, color: "text-green-600", bg: "bg-green-50 dark:bg-green-950/40", text: "Notifications appear here automatically" },
              ].map(({ icon: Icon, color, bg, text }, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <div className={`shrink-0 h-6 w-6 rounded-md flex items-center justify-center ${bg}`}>
          ...
          {totalCount === 0 ? (
            ...
            <Tabs defaultValue="all">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <TabsList>
                  ...
                  <TabsTrigger value="unread" className="gap-1.5 text-xs">
                    Unread
                    {unread.length > 0 && (
                      <Badge className="text-[10px] h-4 px-1.5 bg-primary text-primary-foreground">{unread.length}</Badge>
                    )}
                  </TabsTrigger>
                  <TabsTrigger value="history" className="gap-1.5 text-xs">
                    History
                    <Badge variant="secondary" className="text-[10px] h-4 px-1.5">{read.length}</Badge>
                  </TabsTrigger>
                </TabsList>
              ...
              <TabsContent value="all" className="mt-0">
                {filtered.length === 0
                  ? <EmptyState filtered />
                  : <GroupedList items={filtered} onMarkRead={handleMarkRead} onDelete={handleDelete} />
                }
              </TabsContent>
              ...
              <TabsContent value="unread" className="mt-0">
                {unread.length === 0
                  ...
                  : <GroupedList items={unread} onMarkRead={handleMarkRead} onDelete={handleDelete} />
                }
              </TabsContent>
              ...
              <TabsContent value="history" className="mt-0">
                {read.length === 0
                  ? <EmptyState filtered={!!search} />
                  : <GroupedList items={read} onMarkRead={handleMarkRead} onDelete={handleDelete} />
                }
              </TabsContent>
            </Tabs>
          )}
